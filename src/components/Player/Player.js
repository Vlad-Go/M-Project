...
import YTPlayer from '../../core/YTPlayer';
...
class Player extends AppComponent {
  ...
  get getHTML() {
    ...
    <div class="players__box" id="players__box"></div>`;
  }
  init() {
    super.init();
    this.player = new YTPlayer('players__box');
    this.player.on('Playing', this.onPlay.bind(this));
    this.player.on('Ended', this.playNext.bind(this));
    this.player.on('PlayerReady', this.onPlayerReady.bind(this) );
  ...
  update(itemData) {
    ...
    console.log(this.current, 'update');
    ...
    this.player.source = this.current.id;
    this.pause();
    this.play();
  }
  pause() {
  ...
  onPlayerReady(e) {
    console.log(this.current, 'Ready');
  }
  onPlay(e) {
    const fullTime = this.player.duration;
    this.$fulltime.textContent = timeFormat(fullTime);
