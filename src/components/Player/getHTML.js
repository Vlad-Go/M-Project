export const getHTML = (itemData) =>{
  const img = itemData?.img ?
    `<img class="player__img" src="${itemData.img}" alt="">` :
    `<div class="img-placeholder"></div>`;

  const caption = itemData?.name ?
    `<h4 class="player__caption">${itemData.name}</h4>` :
    `<div class="caption-placeholder"></div>`;

  const author = itemData?.channel ?
    `<p class="player__author">${itemData.channel}</p>` :
